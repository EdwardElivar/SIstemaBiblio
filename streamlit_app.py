...
def pantalla_login():
    ...
    with st.container():
        ...
        st.image("bookia.png", use_container_width=True )
...
def vista_escanear_libro():
    ...
    st.markdown(
        ...
        .full-scan-overlay {
            position: relative;
            margin-top: -440px; /* sube el overlay sobre la cámara */
        ...
        /* 🔧 Permitir clics sobre la cámara */
        .full-scan-overlay,
    ...
    # ===== TARJETA PRINCIPAL =====
    # st.markdown("<div class='scan-wrapper'><div class='scan-card'>", unsafe_allow_html=True)

    st.markdown(
        """
        <div class='scan-badge-wrap'>
            <div class='scan-badge'>
                <span>🧬 Módulo Vision-IA activo</span>
            </div>
        </div>
        <div>
            <p class='scan-subtitle'>
                1️⃣ Toma una foto &nbsp;·&nbsp;
                2️⃣ Identifica con IA &nbsp;·&nbsp;
                3️⃣ Guarda los datos
            </p>
        </div>
    ...
    # ===== CÁMARA + OVERLAY =====
    # st.markdown("##### 📸 Captura la portada del libro", unsafe_allow_html=True)
    ...
    # Overlay visual encima del componente cámara
    st.markdown(
        """
    ...
    if img is not None and scan_clicked:
        ...
        if error:
            ...
            st.error("No se pudieron obtener datos del libro.")
        else:
            st.session_state.scan_data = data
            st.success("✨ Libro identificado. Revisa y ajusta los datos antes de guardar.")
    ...
    if st.session_state.scan_data:
        ...
        st.markdown("<h3>📘 Datos detectados por IA</h3>", unsafe_allow_html=True)
        ...
        try:
            anio_val = int(d.get("anio", 0) or 0)
        except Exception:
            anio_val = 0
        ...
        editorial = st.text_input(
            "Editorial", d.get("editorial", ""), key="scan_editorial"
        )
